...
1. Change detection and the compose file always come from the repo clone (`<stacks_base_dir>/<name>/docker-compose.yml`). `project_directory` (`Stack.ProjectDirectory`, renamed from `working_dir` — see ADR-0045) only sets `--project-directory` (compose project identity + `.env` loading) — never conflate the two. When a stack sets no `project_directory`, it defaults to `<project_directory_base>/<name>` if the host config sets `project_directory_base` (ADR-0045) — a separate base dir from `stacks_base_dir`, since the two rarely coincide (e.g. a NixOS modules dir vs. the deploy repo clone).
