...
1. Change detection and the compose file always come from the repo clone (`<stacks_base_dir>/<name>/docker-compose.yml`). `working_dir` only sets `--project-directory` (compose project identity + `.env` loading) — never conflate the two. When a stack sets no `working_dir`, it defaults to `<working_dir_base>/<name>` if the host config sets `working_dir_base` (ADR-0045) — a separate base dir from `stacks_base_dir`, since the two rarely coincide (e.g. a NixOS modules dir vs. the deploy repo clone).
