...
- Don't duplicate doc content. README.md is a slim landing page (quickstart, features, condensed "How It Works"); the reference lives in `docs/` — `docs/configuration.md` (config + stack fields + vars_file + icons), `docs/nixos.md` (nixos_rebuild + NixOS module + project_directory), `docs/docker.md`, `docs/metrics.md`, `docs/state.md`. Read the relevant page when touching `internal/deploy` or `internal/config`, and keep it in sync.
