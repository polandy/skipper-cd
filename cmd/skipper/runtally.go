...
//
// The two reserved pseudo-stacks need special handling: a NixOS rebuild
// failure and any stack-discovery config failure abort DeployAllStacks
// *before* it reaches PostRunHook (Invariants 4 and 8), so a failed count for
// either would otherwise linger in the tally and get misattributed to
// whichever later run happens to flush next. NixosStateKey's non-failure
// outcomes (deployed/skipped/queued) complete normally within a run and are
// counted like any stack's.
...
func (t *runTally) observe(e events.DeployEvent) {
	if e.Stack == deploy.ConfigStateKey {
		return
	}
