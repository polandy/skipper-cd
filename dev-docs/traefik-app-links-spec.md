...
- Whether the extra `docker inspect` call per stack per poll tick is
  noticeable on a host with many stacks — currently assumed negligible, same
  order of magnitude as the existing health/orphans polling, but worth a
  quick check once implemented.
- No ADR yet — this is a proposal spec. One gets written alongside the
  implementation PR, numbered at merge time by checking live git state (open
  PRs/branches), not reserved in advance.
