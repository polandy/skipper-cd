...
- **`stack_discovery` defaults to `true`** — discovery is the primary model, so
  an omitted key enables it (the deploy repo declares the stacks). Listing the
  stacks in the config yourself is opt-in via an explicit `stack_discovery:
  ...
  `Config` keeps the zero value (false), which the Go test suite relies on.
