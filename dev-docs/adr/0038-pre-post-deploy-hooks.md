...
streaming machinery. Decided with Andy on a clickable mockup (2026-07-19):
(1) a **hooks badge** on the stack cell (newest deploy row + roster row) whose
panel lists the configured commands — commands ride **inline** on the `stacks`
roster snapshot, so the panel opens with no fetch and no new endpoint;
...
`upcoming`); (3) **hook log** reuse — `runHooks` threads the `stack` through the
command context so its child-process lines are attributed in the existing log
view, and the running phase carries the **container-logs console icon** (`clog-btn`,
ADR-0037) that opens the stack-filtered log view (**Option A**: icon on the phase,
shown only while a hook runs, distinct from the persistent per-row container-logs
icon). Full surface in `internal/ui/UI_SPEC.md` (§ Deploy hooks) and
