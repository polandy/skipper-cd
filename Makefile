...
## --- check-host-colors (not a CI job, run standalone) ----------------------
...
# retuned the script without regenerating the other.
check-host-colors:
	node scripts/gen-host-colors.mjs | grep -oE -- '--host-[0-5]:#[0-9a-fA-F]{6}' > /tmp/host-colors-generated.txt
	grep -oE -- '--host-[0-5]:#[0-9a-fA-F]{6}' internal/ui/static/app.css > /tmp/host-colors-committed.txt
	diff /tmp/host-colors-generated.txt /tmp/host-colors-committed.txt
	rm -f /tmp/host-colors-generated.txt /tmp/host-colors-committed.txt
