...
> six status badges), **UA3** (skip-filter toggle + persistence), and **UA4**
> (time-mode toggle + persistence) passing and an `e2e-ui` CI job (behaviour-only).
> Remaining: the rest of mask A (UA5–UA10), masks B–D, and the visual-snapshot
> baselines (§5), at which point `e2e-ui` moves into Playwright's pinned container.
