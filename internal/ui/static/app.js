(function() {
  ...
  // metadata — so the local forge base is the right one everywhere here.
  function shaHTML(sha, testid) {
    return commitLinkHTML(sha, { cls: 'm-sha', base: repoWebURL, title: sha, testid: testid });
  }
  ...
  // nothing to show.
  function renderCommitHead(commits, meta) {
  ...
  // leadHTML renders the change-detection lead, linking the commit it names.
  // watchedSummary stays a pure, unit-tested string: only the SHA token it
  ...
  function createWatchedPanel(stack) {
    ...
    el.innerHTML =
      ...
      `<div class="wp-lead" data-testid="watched-lead">${leadHTML(entry, items.length)}</div>` +
  ...
  // nextTrailHTML renders the look-ahead trail "→ a · b · +N", capping the names
  // shown so a long run does not widen the header without bound.
  ...
  // upcoming snapshot: the active stack leads (ship badge), the rest follow in
  // deploy order. Read-only — no switches.
