...
- **`orphans`** `{ "orphans": [{ "project": "old", "class": "orphaned", "working_dir": "…", "containers": 2, "prunable": true }] }` — compose projects the discovered stack set no longer accounts for (ADR-0036), driving the [Orphans](#orphans) section. Published on connect and on each health-poll tick while a UI client is subscribed; empty when none are found.
