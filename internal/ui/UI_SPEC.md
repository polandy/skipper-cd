...
- **Per-host identity dot** — a small colour dot leading each merged-feed and roster row's stack cell, coloured by the host's palette slot (`assignHostColors`, six per-theme `--host-0…5` slots kept clear of the status hues; deterministic name-hash with collision-avoidance so no two hosts share a colour until the six slots are exhausted). It is the *identity* channel, separate from the status left-bar/badge. The dot doubles as a **quick filter**: clicking it isolates both views to that host, clicking any dot again clears back to all (a toggle complementing the drawer's multi-select). Dots stay visible in multi-host even at one host in view (so a dot is always there to clear the filter); while a filter is active they wear a steady host-colour halo. The dot shows its hostname on hover (`title`) and tap (`data-taptip`), with an enlarged invisible hit area.
- **Merged deploy feed** — local + peer deploy rows interleaved by timestamp. Peer rows are **read-only mirrors** (`.peer-row`): host dot, time, stack, status badge, duration, changed-file **count** — no diff/history/health/logs/hooks drill-down and no expand panel. Local rows are prepended live; peer rows are re-slotted by timestamp after they land.
- **Merged roster** (Stacks view) — local stacks first, then each peer's stacks appended per host, all host-tagged and obeying the same filter. Peer roster rows are read-only (dot + icon + name, status, last deploy, commit); only the peer rows rebuild on a peers refresh, so an open local panel is never collapsed.
...
| `clog-btn` | Console icon that opens a log (ADR-0037) | Container logs on the newest deploy row, each `health-service` line, and each roster row (`data-clog-stack`/`data-clog-service`); the `hook-log-btn` variant on a running `hook-phase` opens the same panel in skipper mode (ADR-0038) |
