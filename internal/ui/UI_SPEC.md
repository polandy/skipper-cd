...
- **`stacks`** `{ "disabled": ["experiments"], "roster": [ { "name": "traefik", "disabled": false, "last_status": "success", "last_at": "…", "last_commit": "a1b2c3d" }, … ] }` — stack-set facts that are not deploy events. `disabled` lists the names parked via `disabled: true` in stack-discovery mode (ADR-0034), driving the [Disabled stacks](#disabled-stacks) line (empty/null in legacy mode). `roster` is the full inventory driving the [Stacks view](#stacks-view) — every declared stack with its last outcome (`last_status` empty = never deployed; disabled entries carry no outcome). Published on connect and after every deploy run.
