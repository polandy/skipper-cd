...
- **`stacks`** `{ "disabled": ["experiments"], "roster": [ { "name": "traefik", "disabled": false, "last_status": "success", "last_at": "…", "last_commit": "a1b2c3d", "hooks": { "pre_deploy": ["docker exec traefik-restic backup"], "post_deploy": [] } }, … ] }` — stack-set facts that are not deploy events. `disabled` lists the names parked via `disabled: true` in stack-discovery mode (ADR-0034), driving the [Disabled stacks](#disabled-stacks) line (empty/null in legacy mode). `roster` is the full inventory driving the [Stacks view](#stacks-view) — every declared stack with its last outcome (`last_status` empty = never deployed; disabled entries carry no outcome). A roster entry carries a `hooks` object with the stack's configured `pre_deploy` / `post_deploy` command lines (omitted when the stack declares none), driving the [hooks badge + panel](#deploy-hooks) with no extra fetch. Published on connect and after every deploy run.
...
Hook stdout/stderr already streams to the [Log view](#log-view) via `/api/logs` (the log pipeline, ADR-0013). For hooks these child-process lines carry a **`stack`** attr (threaded through the command context — `runHooks` knows the stack), so the log view attributes them as a `[stack]` prefix and the stack filter matches them, unlike unattributed `docker`/`git` child output.

The running-hook phase carries a **console icon** — the *same* `clog-btn` glyph the [container logs](#container-logs) feature uses (`data-testid="clog-btn"`) — that switches to the Log view and seeds its [search filter](#log-view) with the stack name. Watching a running hook reuses the log view's follow / search / fullscreen; no new streaming panel. This icon shows **only while a hook runs** (on the `hook-phase`), so it is temporally distinct from the persistent per-row container-logs `clog-btn` in the stack cell — they open different things (the hook's skipper-side output vs. `docker compose logs`) and are rarely visible at the same instant.
...
| `hook-phase` | Phase sub-label on a `deploying` row's status badge | Present only while a hook runs (`pre_deploy hook 1/2` / `post_deploy hook`); carries a `clog-btn` console icon that opens the stack-filtered log view |
