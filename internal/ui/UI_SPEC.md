...
- **Both tables drop the Version column to a second line under the row's identity columns** (the [deploy table](#deploy-table) and the [Stacks roster](#service-versions) alike), header label included, so the remaining columns stay in lockstep with their header. The **status cell spans both lines**, so its health pill sits level with the versions rather than above them — the status column is two lines tall while Time and Stack are one, and a line placed under *everything* left that band of row empty. On that line the chips flow and wrap instead of stacking. An empty cell takes no line.
