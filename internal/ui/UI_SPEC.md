...
A stack that declares any hook carries a small **hooks badge** (`data-testid="hooks-badge"`) — on the **newest deploy row** per stack inside its [⋯ overflow menu](#row-overflow-menu) (Deploys view), and on the **roster row** inline beside the [icon](#stack-icons) / [container-logs icon](#container-logs) (Stacks view). Absent when the stack has no hooks. Its glyph is a **fishing hook**, deliberately distinct from the container-logs icon it sits next to. It shows the split **`pre+post` count** (e.g. `2+1`), not the sum, so the shape of the hooks reads at a glance.
...
While a deploy executes a hook, the stack's row shows the active phase **identically in the Deploys view and the Stacks roster**: the [status badge](#status-badges) gains a phase sub-label (`data-testid="hook-phase"`) reading `pre_deploy hook 1/2` / `post_deploy hook`, and the row's `hooks-badge` gains an **active/pulsing** state (`data-hook-active`) so the badge that says "this stack has hooks" also says "one is running now". In the Deploys view the badge is collapsed inside the [⋯ overflow menu](#row-overflow-menu), so the pulse rides the **`⋯` button** (`more-btn`) — the visible cue stays on screen while the menu is closed; the roster badge pulses in place. The status cell stacks the badge over the phase (the roster status cell does too, so both views match). The phase carries the **logs icon** that opens the hook log (below).
...
| `more-btn` | `⋯` overflow-menu trigger in the stack cell (T3.13) | Newest deploy row per stack only; collapses the row's secondary actions (history, container logs, deploy hooks); `<button>` with `aria-expanded`; `data-hook-active` pulses it while a hook runs; opens `more-pop` |
...
| `clog-btn` | Console icon that opens a log (ADR-0037) | Container logs inside the newest deploy row's `⋯` menu (not `_nixos` — no compose project), on each `health-service` line, and inline on each roster row (`data-clog-stack`/`data-clog-service`); the `hook-log-btn` variant on a running `hook-phase` opens the same panel in skipper mode (ADR-0038) |
...
| `hooks-badge` | Fishing-hook badge (ADR-0038) | Inside the newest deploy row's `⋯` menu, and inline on the roster row; present only when the stack declares hooks; shows the `pre+post` count; `<button>`, opens `hooks-panel`; two-line `title`; `data-hook-active` while a hook runs (in Deploys the `more-btn` carries the visible pulse) |
