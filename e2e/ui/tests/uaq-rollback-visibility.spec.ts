...
// keeps the WARN-level rollback outcome under the errors chip (UAQ5). UAQ6
// takes its own clean instance: with nothing to report the badge must take
// up no space at all.
...
const statusChip = (page: Page, status: string) =>
  page.locator(
    `[data-testid="deploy-status-filter"] .status-chip[data-status="${status}"]`,
  );
...
test.describe("Maske AQ: rollback visibility after the fact", () => {
  ...
  test("UAQ1: the retry note pairs the success with its rollback", async ({
    page,
    skipper,
  }) => {
    ...
    const noteRow = page.locator(
      '[data-testid="deploy-row"]:has([data-testid="retry-note"])',
    );
    await expect(noteRow).toHaveAttribute("data-status", "success");
    const rollbackId = await deployRow(page, "rolled_back").getAttribute(
      "data-event-id",
    );
  ...
  test("UAQ2: status chips narrow the deploy log and Esc clears them", async ({
    ...
    await expect(
      page.locator('[data-testid="deploy-status-filter"]'),
    ).toBeVisible();
    await expect(
      statusChip(page, "rolled_back").locator(".sc-count"),
    ).toHaveText("1");
    await expect(statusChip(page, "success").locator(".sc-count")).toHaveText(
      "2",
    );
    ...
    await expect(statusChip(page, "rolled_back")).toHaveAttribute(
      "aria-pressed",
      "true",
    );
  ...
  test("UAQ3: the incident badge toggles the pre-filtered Deploys view", async ({
    ...
    await expect(
      page.locator('[data-testid="incident-badge-count"]'),
    ).toHaveText("1");
    ...
    await expect(
      page.locator('[data-testid="deploy-filter-wrap"]'),
    ).toHaveClass(/revealed/);
    await expect(statusChip(page, "rolled_back")).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    await expect(statusChip(page, "failed")).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    await expect(deployRow(page, "rolled_back")).toBeVisible();
  ...
  // failure mode.
  test("UAQ5: the errors chip keeps the rollback outcome line", async ({
    page,
    skipper,
  }) => {
    await rollBackThenRetry(page, skipper);
    ...
    await page
      .locator('[data-testid="view-toggle"] button[data-view="logs"]')
      .click();
    // The rollback outcome: WARN on the record, errors-tier for the filter.
    const outcome = page.locator(
      '.log-line[data-level="WARN"][data-sev="ERROR"]',
    );
...
// UAQ6 — the counterpart to UAQ3: with no bad outcome in the window the badge
// is not just empty, it is gone. Its own `display` beats the UA `[hidden]`
// rule, so hiding at zero has to be said explicitly in CSS; without that line
// an empty rollback-tinted pill sits in the header of every instance with a
// clean 24h. AQ's own instance always rolls back, so this needs a clean one.
test.describe("Maske AQ: a clean window", () => {
  test.use({ startOptions: { stacks: ["web"] } });

  test("UAQ6: the incident badge occupies no space with nothing to report", async ({
    page,
    skipper,
  }) => {
    await page.goto(`${skipper.baseURL}/`);
    // Positive signal: the deploy settled and the header is painted, so the
    // badge's absence is a decision rather than an unrendered page.
    await expect(deployRow(page, "success")).toHaveCount(1);
    await expect(page.locator('[data-testid="view-toggle"]')).toBeVisible();
    const badge = page.locator('[data-testid="incident-badge"]');
    await expect(badge).toBeHidden();
    await expect(badge).toHaveJSProperty("offsetWidth", 0);
  });
});
