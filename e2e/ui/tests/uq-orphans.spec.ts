...
test.use({
  startOptions: { stacks: ["web", "api"], discovery: {}, healthPoll: 1 },
});
...
function orphanListing(base: string): OrphanContainer[] {
  return [
    ...
    // Removed stack, still running under stacks_base_dir -> orphaned, prunable.
    {
      ...
      workingDir: `${base}/legacy-cache`,
      name: "legacy-cache-redis-1",
      service: "redis",
      image: "redis:7",
      state: "running",
      status: "Up 5 days",
...
test("UQ1: lists orphaned + unmanaged projects, expandable to containers and data-safety facts", async ({
  ...
  // A formerly-managed project is present, so the section opens itself rather
  // than hiding the removed stack behind a collapsed header.
  await expect(section(page)).toHaveClass(/\bopen\b/);
  await expect(head(page)).toHaveAttribute("aria-expanded", "true");
  await expect(count(page)).toHaveClass(/\bhas-orphaned\b/);
  await expect(item(page, "legacy-cache")).toHaveAttribute(
    "data-class",
    "orphaned",
  );
  await expect(item(page, "portainer")).toHaveAttribute(
    "data-class",
    "unmanaged",
  );

  // The header is still a manual toggle on top of that.
  await head(page).click();
  await expect(section(page)).not.toHaveClass(/\bopen\b/);
  await head(page).click();
  await expect(section(page)).toHaveClass(/\bopen\b/);
  ...
  await expect(legacy).toContainText(
    `${skipper.stacksBaseDir}/legacy-cache/docker-compose.yml`,
  );
...
test("UQ3: a manually expanded orphan stays open across an orphan-data refresh", async ({
  ...
  skipper.setOrphans(orphanListing(skipper.stacksBaseDir));
  await page.goto(`${skipper.baseURL}/`);
  await expect(section(page)).toHaveClass(/\bopen\b/); // auto-opened by the orphan
  const legacy = item(page, "legacy-cache");
  ...
  changed.find((c) => c.name === "legacy-cache-worker-1")!.status =
    "Exited (0) 3 days ago";
