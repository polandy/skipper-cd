...
import { manifestVersion, repoRoot, skipperBinPath } from './fixtures/harness';
...
//
// The build injects the version via -ldflags from .release-please-manifest.json,
// exactly like the Docker and Nix builds, so the header version label (UD5)
// renders the real deployed version rather than the "dev" fallback.
export default function globalSetup() {
  if (process.env.SKIPPER_E2E_BIN) return; // caller supplied a binary
  const bin = skipperBinPath();
  mkdirSync(dirname(bin), { recursive: true });
  execFileSync('go', ['build', '-ldflags', `-X main.version=${manifestVersion()}`, '-o', bin, './cmd/skipper'], {
    cwd: repoRoot(),
    stdio: 'inherit',
  });
}
